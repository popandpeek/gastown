...
func executeSling(params SlingParams) (*SlingResult, error) {
	...
	if (info.Status == "pinned" || info.Status == "hooked" || info.Status == "working") && !params.Force {
		// Auto-force when hooked/working agent's session is confirmed dead (gt-npzy, GH#1380).
	...
	if (info.Status == "hooked" || info.Status == "working") && params.Force && info.Assignee != "" {
